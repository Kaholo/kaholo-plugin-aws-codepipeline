...
function listAuto(listFuncName, outputName, fields = []) {
  return async (query, pluginSettings, triggerParameters) => {
    ...
    const client = CodePipelineService.from(params, settings);
    var nextToken, items = [];
    do {
...
async function listRegions(query, pluginSettings, actionParams) {
  ...
  let client;
  try {
    client = CodePipelineService.from(params, settings);
  } catch (e) {
...
module.exports = {
  ...
  listS3BucketsAuto: listAuto("listS3Buckets", "Buckets", ["Name"]),
  listKmsKeysAuto: listAuto("listKmsKeys", "Keys", ["KeyId"]),
  listPipelinesAuto: listAuto("listPipelines", "pipelines", ["name"])
}
